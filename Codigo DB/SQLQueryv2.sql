...
as
begin
	select U.codigo AS ID_Usuario, U.nombre AS Nombre, U.correo AS Correo, S.idrol AS ID_Rol, R.rolename AS Nombre_Rol
	from UsersAD U
	inner join UsersRoles S ON U.codigo = S.iduser
	INNER JOIN Roles R ON R.codigo = s.idrol
...
--MODIFICAR PASSWORD USUARIO POR CORREO
-------------------------------
...
BEGIN
		...
		IF @ROL = 'admin'
			BEGIN
				SELECT t.TecnicoID AS [Cogido Tecnico], t.Nombre, a.AsignacionID AS [Codigo Asignacion], a.FechaAsignacion AS [Fecha Asignacion], 
				a.ReparacionID AS [Codigo Reparacion], r.FechaSolicitud AS [Fecha Solicitud], r.Estado, dr.Descripcion, 
				e.TipoEquipo AS [Tipo de Equipo], e.Modelo, u.Nombre [Nombre de Usuario], u.Correo
				FROM Tecnicos t
		...
		ELSE
			BEGIN
				SELECT t.TecnicoID AS [Cogido Tecnico], t.Nombre, a.AsignacionID AS [Codigo Asignacion], a.FechaAsignacion AS [Fecha Asignacion],
				a.ReparacionID AS [Codigo Reparacion], r.FechaSolicitud, r.Estado AS [Fecha Solicitud], dr.Descripcion, e.TipoEquipo AS [Tipo de Equipo],
				e.Modelo, u.Nombre AS [Nombre de Usuario], u.Correo
				FROM Tecnicos t
				inner join Asignaciones a on t.TecnicoID = a.TecnicoID
				inner join Reparaciones r on r.ReparacionID = a.ReparacionID
				inner join DetallesReparacion dr on dr.ReparacionID = a.ReparacionID
...
--------------------------------------------------------------------------------------
--INNER JOIN CONSULTA ESPECIFICA CONSULTARTECNICOS
---

CREATE PROCEDURE ConsultarTecnicoEspecifico
@ROL VARCHAR(20)='',
@TID INT = 0
AS
BEGIN
